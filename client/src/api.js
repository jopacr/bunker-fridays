...
export const api = {
  ...
  addBlackout: (date, reason, dateTo) => req("POST", "/me/blackouts", { date, reason, ...(dateTo ? { dateTo } : {}) }),
  removeBlackout: (date, reason) => req("DELETE", "/me/blackouts", { date, reason }),
  adminAddBlackout: (artistId, date, reason, dateTo) => req("POST", `/admin/artists/${artistId}/blackouts`, { date, reason, ...(dateTo ? { dateTo } : {}) }),
  adminRemoveBlackout: (artistId, date, reason) => req("DELETE", `/admin/artists/${artistId}/blackouts`, { date, reason }),
  ...
  setRequestSetType: (reqId, setType) => req("POST", `/admin/requests/${reqId}/settype`, { setType }),
  draftNightPromo: (date) => req("POST", `/admin/nights/${date}/promo`),
  reviewNight: (date, reviewed) => req("POST", `/admin/nights/${date}/review`, { reviewed }),
  ...
  softPassRecommend: (payload) => req("POST", "/admin/recommend/soft-pass", payload),
  clearSoftPass: (artistId, date, weeks) => req("DELETE", "/admin/recommend/soft-pass", { artistId, date, weeks }),
  whyNotEligible: (artistId, date) => req("GET", `/admin/recommend/why?artistId=${encodeURIComponent(artistId)}&date=${encodeURIComponent(date)}`),
  outreach: (payload) => req("POST", "/admin/recommend/outreach", payload),
  setTentative: (artistId, date, slotLabel, setType, weeks) => req("POST", "/admin/recommend/tentative", { artistId, date, slotLabel, setType, weeks }),
  clearTentative: (artistId, date, weeks) => req("DELETE", "/admin/recommend/tentative", { artistId, date, weeks }),
  confirmTentative: (artistId, date, slotTime, setType) => req("POST", "/admin/recommend/tentative/confirm", { artistId, date, slotTime, setType }),
